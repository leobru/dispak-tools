...
std::string
unicode_to_utf8(unsigned short ch) {
    if (ch < 0x80) {
        return std::string() + char(ch);
    }
    if (ch < 0x800) {
        return (std::string() + char(ch >> 6 | 0xc0)) + char((ch & 0x3f) | 0x80);
    }
    return ((std::string() + char(ch >> 12 | 0xe0)) + char(((ch >> 6) & 0x3f) | 0x80)) + char((ch & 0x3f) | 0x80); 
}
...
std::string gost_to_utf8(unsigned char ch) {
    return unicode_to_utf8(gost_to_unicode(ch));
}
